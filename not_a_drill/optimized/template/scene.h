...
// -----------------------------------------------------------
// PlaneCompileTime primitive
// Basic infinite plane, defined by a normal and a distance
// from the origin (in the direction of the normal).
// Albedo implementation known at compile time thus branchless.
// -----------------------------------------------------------
enum class Albedo
{
	Floor,
...
template<Albedo A>
class PlaneCompileTime
{
public:
	PlaneCompileTime() = default;
	PlaneCompileTime(int idx, float3 normal, float dist) : N(normal), d(dist), objIdx(idx) {}
	...
	float3 GetAlbedo(const float3 I) const
	{
		constexpr float InvC = 1.0f / 255.f;

		// No switch at runtime thus branchless
		switch (A)
		{
		case Tmpl8::Albedo::Floor:
		{
			// floor albedo: checkerboard
			int ix = (int)(I.x * 2 + 96.01f);
			int iz = (int)(I.z * 2 + 96.01f);
		...
		case Tmpl8::Albedo::Back:
		{
			...
			uint p = logo.pixels[(ix & 127) + (iy & 63) * 128];
			uint3 i3((p >> 16) & 255, (p >> 8) & 255, p & 255);
			return float3(i3) * InvC;
		}
		case Tmpl8::Albedo::Left:
		{
			// left wall: red
			static Surface red("../assets/red.png");
			int ix = (int)((I.z - 4) * (512.0f / 7)), iy = (int)((2 - I.y) * (512.0f / 3));
...
public:
	...
	void Intersect( Ray& ray ) const
	{
		...
		const float tmax = HorizontalMin3SSE(vmax4);
		const float tmin = HorizontalMax3SSE(vmin4);
...
public:
	...
	void Intersect( Ray& ray ) const
	{
		...
		const __m128 O = TransformPositionSSE_128( ray.O4, invT );
		const __m128 D = TransformVectorSSE_128( ray.D4, invT );
		// extension rays need double precision for the quadratic solver!
		double po = 1, m = Dot3SSE( O, O ), k3 = Dot3SSE( O, D ), k32 = k3 * k3;
		// bounding sphere test
		double v = k32 - m + r2;
		if (v < 0) return;
		// setup torus intersection
		double k = (m - rt2 - rc2) * 0.5, k2 = k32 + rc2 * D.m128_f32[2] * D.m128_f32[2] + k;
		double k1 = k * k3 + rc2 * O.m128_f32[2] * D.m128_f32[2], k0 = k * k + rc2 * O.m128_f32[2] * O.m128_f32[2] - rc2 * rt2;
...
public:
	Scene()
	{
		...
		p1 = PlaneCompileTime<Albedo::Left>(4, float3(1, 0, 0), 3);           // 4: left wall
		p2 = PlaneCompileTime<Albedo::Right>(5, float3(-1, 0, 0), 2.99f);		// 5: right wall
		p3 = PlaneCompileTime<Albedo::Floor>(6, float3(0, 1, 0), 1);			// 6: floor
		p4 = PlaneCompileTime<Albedo::Default>(7, float3(0, -1, 0), 2);			// 7: ceiling
		p5 = PlaneCompileTime<Albedo::Default>(8, float3(0, 0, 1), 3);			// 8: front wall
		p6 = PlaneCompileTime<Albedo::Back>(9, float3(0, 0, -1), 3.99f);		// 9: back wall
	...
	float3 GetAlbedo( int objIdx, float3 I ) const
	{
		...
		{
		case 0:
			return p1.GetAlbedo(I);
		case 1:
			return p2.GetAlbedo(I);
		case 2:
			return p3.GetAlbedo(I);
		case 3:
			return p4.GetAlbedo(I);
		case 4:
			return p5.GetAlbedo(I);
		case 5:
			return p6.GetAlbedo(I);
		}
	...
	PlaneCompileTime<Albedo::Left> p1;
	PlaneCompileTime<Albedo::Right> p2;
	PlaneCompileTime<Albedo::Floor> p3;
	PlaneCompileTime<Albedo::Default> p4;
	PlaneCompileTime<Albedo::Default> p5;
	PlaneCompileTime<Albedo::Back> p6;
